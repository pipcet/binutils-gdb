/* tc-wasm.c -- "Assembler" code for the asm.js target
...
#define WASM_INSN(NAME, CONSTR, OPCODE, SIZE, ISA, BIN) \
{#NAME, CONSTR, OPCODE, SIZE, ISA, BIN},

struct wasm_opcodes_s wasm_opcodes[] =
{
...
/* WASM target-specific switches.  */
struct wasm_opt_s
{
  int all_opcodes;  /* -mall-opcodes: accept all known WASM opcodes.  */
...
static struct wasm_opt_s wasm_opt = { 0, 0, 0, 0 };
...
/* Opcode hash table.  */
static struct hash_control *wasm_hash;
...
md_begin (void)
{
  ...
  /* Insert unique names into hash table.  This hash table then provides a
     ...
     table.  */
  for (opcode = wasm_opcodes; opcode->name; opcode++)
    hash_insert (wasm_hash, opcode->name, (char *) opcode);

  linkrelax = !wasm_opt.no_link_relax;
...
int
wasm_validate_fix_sub (fixS *fix ATTRIBUTE_UNUSED)
{
  return 1;
}
...
bfd_boolean
wasm_allow_local_subtract (expressionS * left ATTRIBUTE_UNUSED,
...
void
wasm_handle_align (fragS *fragP ATTRIBUTE_UNUSED)
{
}

void
wasm_post_relax_hook (void)
{
}

void wasm_elf_final_processing (void)
{
}

int
wasm_force_relocation (fixS *f ATTRIBUTE_UNUSED)
{
  return 1;
}
...
void wasm_start_line_hook (void)
{
  if (!is_pseudo_line () &&
      !is_label_line () &&
      !is_noapp_line () &&
