...
#define TARGET_FORMAT "elf32-wasm"

/* This macro is the BFD architecture to pass to `bfd_set_arch_mach'.  */
#define TARGET_ARCH bfd_arch_wasm
...
//#define TC_FORCE_RELOCATION(fix)	wasm_force_relocation (fix)
extern int wasm_force_relocation (struct fix *);
...
//#define TC_VALIDATE_FIX_SUB(fix, seg)   wasm_validate_fix_sub (fix)
extern int wasm_validate_fix_sub (struct fix *);
...
/* 32 bits pseudo-addresses are used on WASM.  */
...
#define md_allow_local_subtract(l,r,s) wasm_allow_local_subtract (l, r, s)
extern bfd_boolean wasm_allow_local_subtract (expressionS *, expressionS *, segT);

#define elf_tc_final_processing 	wasm_elf_final_processing
extern void wasm_elf_final_processing (void);

#define md_post_relax_hook wasm_post_relax_hook ()
extern void wasm_post_relax_hook (void);

extern void wasm_start_line_hook (void);
#define md_start_line_hook() wasm_start_line_hook ()

#define HANDLE_ALIGN(fragP) wasm_handle_align (fragP)
extern void wasm_handle_align (fragS *fragP);

struct wasm_frag_data
{
  unsigned is_org : 1;
  unsigned is_align : 1;
  unsigned has_fill : 1;
...
#define TC_FRAG_TYPE			struct wasm_frag_data
